...
#boxplots showing that ASD and TD kids are similar on week 1, but TD kids develop more overall
ggplot(Data) +
  geom_boxplot(aes(y = types_CHI, x = Diagnosis)) +
  facet_wrap(~ VISIT)
ggplot(Data) +
  geom_boxplot(aes(y = tokens_CHI, x = Diagnosis)) +
  facet_wrap(~ VISIT)
ggplot(Data) +
  geom_boxplot(aes(y = CHI_MLU, x = Diagnosis)) +
  facet_wrap(~ VISIT)

#Boxplot of IQ levels. They show that kids with ASD differ alot more on verbal IQ but for non verbal IQ the two groups are similar.
ggplot(Data) +
  geom_boxplot(aes(y = verbalIQ, x = Diagnosis))
ggplot(Data) +
  geom_boxplot(aes(y = nonVerbalIQ, x = Diagnosis))
...
```{r}
m1 = lmer(CHI_MLU ~ Diagnosis + Gender + VISIT + (1 + ID|VISIT),Data)
summary(m1)
```
...
```{r}
m1_null = lmer(CHI_MLU ~ Diagnosis + (1 + ID|VISIT),Data)
anova(m1,m1_null)
```
