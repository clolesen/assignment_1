...
```{r}
setwd("~/Documents/skole/CogSci/3rd semester/exp meth 3/assignments/1/assignment1")
...
```{r}
data_demo = read.csv("demo_train.csv")
data_LU = read.csv("LU_train.csv")
data_token = read.csv("token_train.csv")
```
...
```{r}
library(data.table)
setnames(data_demo, "Child.ID", "SUBJ")
setnames(data_demo, "Visit", "VISIT")
```
...
data_LU$VISIT = str_extract(data_LU$VISIT, "\\d")
data_token$VISIT = str_extract(data_token$VISIT, "\\d")
...
```{r}
data_demo$SUBJ = gsub("\\.", "", data_demo$SUBJ)
data_LU$SUBJ = gsub("\\.", "", data_LU$SUBJ)
data_token$SUBJ = gsub("\\.", "", data_token$SUBJ)
```
...
```{r}
library(dplyr)
subdata_demo = select(data_demo, SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS,  MullenRaw, ExpressiveLangRaw)

subdata_LU = select(data_LU, SUBJ, VISIT, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)

subdata_token = select(data_token, SUBJ, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)

setnames(subdata_demo,"MullenRaw", "nonVerbalIQ")
setnames(subdata_demo,"ExpressiveLangRaw", "verbalIQ")
```
...
```{r}
temp_merge = merge(subdata_demo, subdata_LU, by=c("SUBJ", "VISIT"))
allData = merge(temp_merge, subdata_token, by=c("SUBJ", "VISIT"))
```
...
```{r}
temp_subset = subset(allData,VISIT==1, select = c("SUBJ", "ADOS", "nonVerbalIQ", "verbalIQ"))
temp_subset$ID = 1:nrow(temp_subset)
allData$ADOS = NULL
allData$nonVerbalIQ = NULL
allData$verbalIQ = NULL
allData = merge(temp_subset, allData, by="SUBJ")
allData$SUBJ = NULL
```
...
```{r}
#first part was done above

allData$VISIT = as.numeric(allData$VISIT)

library(plyr)
allData$Gender = mapvalues(allData$Gender, from = c("1", "2"), to = c("M", "F"))
allData$Diagnosis = mapvalues(allData$Diagnosis, from = c("A", "B"), to = c("ASD", "TD"))
```
...
```{r}
write.csv(allData, file = "MergedData.csv")
```
...
filter1 = allData %>%
  group_by(ID) %>%
  summarise(visitMLU = mean(CHI_MLU, na.rm = TRUE)) %>%
  filter(visitMLU > 2.7)

filter2 = filter(allData, CHI_MLU < 1.5, VISIT == 1)

filter3 = allData %>%
  group_by(ID) %>%
  summarise(visitSUM = sum(VISIT)) %>%
  filter(visitSUM != 21)
...
sel1 = allData %>%
  ...
  select(ID, Diagnosis, contains("MLU"), contains("token"))

sel2 = select(allData, ID, VISIT, ID, VISIT)
...
MSP1 = allData %>%
  group_by(VISIT) %>%
  summarise(visit_mean = mean(tokens_CHI)) %>%
  merge(allData, by = "VISIT") %>%
  arrange(ID)

MSP2 = allData %>%
  group_by(ID) %>%
  summarise(child_mean = mean(tokens_CHI)) %>%
  merge(MSP1, by = "ID")

#MSP3 - we want the data so that we can look at the development over time.
